...
console.log('Emscripten version: 3.1.39');
console.log('Emscripten configuration: ');

const scriptDir = 'game/';
...
var Module = {
  ...
  UE4_indexedDBName: 'UE4_assetDatabase_DrumSequencer', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
  UE4_indexedDBVersion: 202403182013, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments =
  sessionStorageCommandLine != null
    ? [sessionStorageCommandLine]
    : ['../../../DrumSequencer/DrumSequencer.uproject', '-stdout'];
...
function addLog(info, color) {
  document.querySelector('#logwindow').innerHTML =
...
if (!enableReadFromIndexedDB)
  console.warn('Running with IndexedDB access disabled.');
else if (!enableWriteToIndexedDB)
  console.warn('Running in read-only IndexedDB access mode.');
...
function reportDataBytesStoredInIndexedDB(deltaBytes) {
  ...
  document.getElementById('clear_indexeddb').title =
    'Clear IndexedDB (' +
...
Module.locateFile = function (name) {
  ...
  return serveGzipped ? scriptDir + name + '.gz' : scriptDir + name;
};
  ...
  return new Promise(function (resolve, reject) {
        ...
        if (
          ...
          var readableStream =
            xhr.responseType == 'blob'
              ? xhr.response.stream()
              : new Response(xhr.response).body;
          //console.warn(readableStream);
          var decompressedReadableStream = readableStream.pipeThrough(
    ...
    xhr.onprogress = function (p) {
      reportDownloadProgress(
        url,
        p.loaded,
        p.lengthComputable ? p.total : contentLength
      );
    };
          ...
          if (xhr.getResponseHeader('Content-Encoding') != 'gzip') {
            ...
            if (
              ...
              taskFinished(
                ...
                'Downloaded a compressed file ' +
                  url +
                  ' without the necessary HTTP response header "Content-Encoding: gzip" specified!<br>Please configure gzip compression on this asset on the web server to serve gzipped assets!<br>This browser does not appear to support DecompressionStream so decompression will not be attempted as a workaround.'
              );
              xhr.onload =
                xhr.onprogress =
                xhr.onerror =
          ...
          if (
            ...
            console.warn(
              'Downloaded a compressed file ' +
                ...
                '"!\nPlease set the MIME type of the asset to "' +
...
function postRunEmscripten() {
  ...
  document.querySelector('#compilingmessage').remove();
...
ready(() => {
  ...
  function withIndexedDB(db) {
    ...
    var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule')
      ...
      .catch(function () {
        return download(
          Module.locateFile('DrumSequencer-HTML5-Shipping.wasm'),
          'arraybuffer'
    ...
    var mainJsDownload = fetchOrDownloadAndStore(
      db,
      Module.locateFile('DrumSequencer-HTML5-Shipping.js'),
      'blob'
    ).then(function (data) {
    ...
    var dataJsDownload = fetchOrDownloadAndStore(
      db,
      Module.locateFile('DrumSequencer-HTML5-Shipping.data.js')
    );
    ...
    var dataDownload =
      /* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('DrumSequencer-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('DrumSequencer-HTML5-Shipping.data')] = dataArrayBuffer;
...
*/
      ...
      fetchOrDownloadAndStore(
        db,
        Module.locateFile('DrumSequencer-HTML5-Shipping.data'),
        'arraybuffer'
